...
# than generic over T.
#
# `List` didn't support bracket-write at all when this file was first
# written (`PromethiumBaseLibrary.List` only declared `__getitem__`, so the
# compiler correctly exposed a get-only indexer) — this module and several
# others (`Counter.py`/`OrderedDict.py`/`DefaultDict.py`) worked around it
# with `pop(index)` + `insert(index, value)`. `List` has since gained a
# real `__setitem__`, so this now uses plain bracket assignment
# (`heap[position] = value`) directly instead of a `_set` helper.
#
...
def nsmallest(n: int, values: List[int]) -> List[int]:
    ordered: List[int] = Promethium.sorted(values)
...
def nsmallest(n: int, values: List[float]) -> List[float]:
    ordered: List[float] = Promethium.sorted(values)
    limit: int = _clamp(n, len(ordered))
...
def nsmallest(n: int, values: List[str]) -> List[str]:
    ordered: List[str] = Promethium.sorted(values)
    limit: int = _clamp(n, len(ordered))
...
def _siftup(heap: List[int], pos: int):
    ...
    while childpos < endpos:
        ...
        if rightpos < endpos and not (heap.__getitem__(childpos) < heap.__getitem__(rightpos)):
            childpos = rightpos
        heap[position] = heap.__getitem__(childpos)
        position = childpos
